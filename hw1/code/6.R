...
{
  s = sqrt(2*beta/n);
  (gamma((v+1)/2)/(gamma(v/2))) * 1/(sqrt(pi)*s) * ((1+t^2/(v*s))^(-(v+1)/2));
...
samples<-seq(from=-10,to=100,length.out=10^2)
...
sample_mean.cntrl  = (1/ngroup.cntrl)*(sum(group1));
sample_variance.cntrl = ( ((sigma0^2)*n0) + (sample_variance.cntrl*ngroup.cntrl) ) /(ngroup.cntrl+n0);
...
plot(samples, samples.mean1, type='l', col='red', lwd=1,
      ylim=c(0,0.012),xlim=c(2000,3200),
...
abline(v = (mun_cntrl), col='red', lwd = 3, lty = 2)
abline(v = (mun_mild), col='blue', lwd = 3, lty = 2)
abline(v = (mun_dem), col='green', lwd = 3, lty = 2)
...
cat(paste("P(d_12 < 0) =", (sum(samples.d12 < 0) / length(samples.d12)), "\n"));
cat(paste("P(d_13 < 0) =", (sum(samples.d13 < 0) / length(samples.d13)), "\n"));
cat(paste("P(d_23 < 0) =", (sum(samples.d23 < 0) / length(samples.d23)), "\n"));
